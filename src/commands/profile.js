...
exports.run = async (client, message, args, level) => {
  ...
  try {
    if (args[0] == "create") {
      if (!message.guild) {
        if (client.logins.set(message.author.id)) {
          let passwordRaw = await client.awaitReply(
            message,
            "What do you want as your password?"
          );
          let passwordRawConfirm = await client.awaitReply(
            message,
            "Confirm your password"
          );
          if (passwordRaw !== passwordRawConfirm)
            return message.channel.send(
              "Your password does not match the confirmation."
            );
          let password = crypto
            .createHash("sha256")
            .update(passwordRawConfirm)
            .digest("hex");
        ...
        message.channel.send(
          "Profile creation is unavailable via a guild. Please run this command in DM's."
        );
      ...
      if (!message.guild) {
        ...
        if (password == client.logins.get(message.author.id)) {
          ...
          msg.edit(
            "I could not find a user with that password. (If you need help creating an account try using createprofile instead)"
          );
      } else
        message.channel.send(
          "Profile login is unavailable via a guild. Please run this command in DM's."
        );
    }
    ...
    if (args[0] == "logout") {
      ...
      if (!message.guild) {
        if (args[1] == "change") {
          if (client.logins.set(message.author.id)) {
            let passwordRaw = await client.awaitReply(
              message,
              "What do you want as your new password?"
            );
            let oldPasswordRaw = await client.awaitReply(
              message,
            ...
            if (passwordRaw !== oldPasswordRaw)
              return message.channel.send(
                "Your new password does not match the old password."
              );
            if (passwordRaw !== passwordRawConfirm)
              return message.channel.send(
          ...
          message.reply("You have to use profile password change instead!");
      } else
        message.channel.send(
          "Profile password is unavailable via a guild. Please run this command in DM's."
        );
    } else if (args[0] == "edit") {
      if (client.liusers.get(message.author.id)) {
        if (args[1] == "nickname") {
          ...
          client.profiles.set(
            message.author.id + "hobbies",
            args.slice(2).join(" ")
          );
          msg.edit("Hobbies updated!");
        } else
          message.reply(
            "You have to use profile edit nickname/bio/hobbies instead!"
          );
      ...
      let profile = {
        nickname: client.profiles.get(message.author.id + "nickname"),
